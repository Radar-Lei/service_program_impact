...
def create_markdown_report(program_metrics, stats_results, regression_model):
    """Create a markdown report of the analysis"""
    print("Generating markdown report...")
    
    report = """# Impact Analysis of Metro Service Programs on Passenger Experience

## 1. Introduction

This research analyzes the impact of various service improvement programs implemented by the Shenzhen Metro on passenger experience. By examining passenger feedback collected from social media, the study employs sentiment analysis methodologies to explore the relationship between service initiatives and passenger satisfaction.
...
1. **Service Program Data**: Detailed information on 23 distinct service improvement initiatives implemented by the Shenzhen Metro
2. **Passenger Feedback Data**: Social media feedback associated with each service program, including sentiment analysis scores

### 2.2 Analytical Approach

This analysis employs the following methods:

1. **Sentiment Metrics Calculation**:
   - Net Sentiment = Positive Sentiment Score - Negative Sentiment Score
   - Sentiment Ratio = Positive Sentiment Score / (Positive Sentiment Score + Negative Sentiment Score)

2. **Program Categorization**: Classifying service initiatives into distinct categories for comparative analysis

3. **Statistical Analysis**:
   - ANOVA testing for differences between program categories
   - Tukey HSD tests to identify significantly different program pairs

4. **Regression Analysis**: Exploring the relationship between program characteristics and passenger satisfaction

## 3. Results

### 3.1 Overall Program Sentiment Scores

The following presents service programs ranked by net sentiment score (positive minus negative):

"""
    
    # Add top and bottom 5 programs
    top_programs = program_metrics.sort_values('net_sentiment_mean', ascending=False).head(5)
    bottom_programs = program_metrics.sort_values('net_sentiment_mean').head(5)
    
    report += "**Top 5 Service Programs by Sentiment Score:**\n\n"
    report += "| Rank | Service Program | Net Sentiment | Program Category |\n"
    report += "|------|----------------|---------------|------------------|\n"
    
    for i, (_, row) in enumerate(top_programs.iterrows()):
        report += f"| {i+1} | {row['Service improvement programs']} | {row['net_sentiment_mean']:.4f} | {row['program_category']} |\n"
...
Different categories of service programs exhibit varying impacts on passenger experience:
...
"""
    ...
    category_metrics = category_metrics.sort_values('net_sentiment_mean', ascending=False)
    
    report += "| Program Category | Average Net Sentiment | Average Positive | Average Negative | Program Count |\n"
    report += "|------------------|----------------------|------------------|------------------|---------------|\n"
    
    for idx, row in category_metrics.iterrows():
        report += f"| {idx} | {row['net_sentiment_mean']:.4f} | {row['Positive_mean']:.4f} | {row['Negative_mean']:.4f} | {int(row['program_count'])} |\n"
...
"""
    ...
    # Add ANOVA results
    f_stat = stats_results['anova_f_statistic']
    p_value = stats_results['anova_p_value']
    
    report += f"**ANOVA Analysis for Program Categories**:\n\n"
    report += f"- F-statistic: {f_stat:.4f}\n"
    report += f"- p-value: {p_value:.4f}\n"
    report += f"- Conclusion: {'Significant differences exist between program categories' if p_value < 0.05 else 'No significant differences between program categories'}\n\n"
    
    report += """### 3.4 Regression Analysis Results

Impact of program characteristics on passenger satisfaction:

"""
    
    # Add regression results summary
    report += "**Influence of Program Categories on Net Sentiment Score**:\n\n"
    report += "```\n"
    report += str(regression_model.summary().tables[1])
    report += "\n```\n\n"
    
    report += f"- R-squared: {regression_model.rsquared:.4f}\n"
    report += f"- Adjusted R-squared: {regression_model.rsquared_adj:.4f}\n"
...
1. **Temperature control initiatives** demonstrate the strongest impact on passenger experience, indicating the high importance of comfort factors
2. **Ticketing services and pricing** programs also receive positive feedback, particularly improvements in payment convenience
3. **Information service** programs show varied evaluations, suggesting targeted improvements are necessary

### 4.2 Policy Recommendations

1. **Expand temperature control programs**: Consider implementing differentiated temperature zones across more metro lines
2. **Improve low-performing programs**: Adjust and enhance programs with below-average sentiment scores
3. **Integrate category advantages**: Develop comprehensive service improvement initiatives that combine strengths from multiple categories

## 5. Conclusion

This research, through analysis of the impact of Shenzhen Metro service programs on passenger experience, reveals significant differences in how various categories of service improvements affect passenger satisfaction. Temperature control, ticketing services, and convenience facilities generally receive higher evaluations, while certain personnel services and information service programs require further refinement. These findings provide data-driven support for optimizing future service initiatives and resource allocation for the Shenzhen Metro system.
...
"""
    ...
    with open('service_program_impact_report.md', 'w', encoding='utf-8') as f:
        f.write(report)
...
def main():
    ...
    # 7. Generate markdown report
    create_markdown_report(program_metrics, stats_results, regression_model)
